...
export default function Admin() {
  ...
  const cargarMensajes = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setMensajes(data);
      setMensajeActivo((actual) => {
        if (actual) {
          const encontrado = data.find((m) => m.id === actual.id);
          return encontrado || data[0] || null;
        }
        return data[0] || null;
      });
    } catch (err) {
...
const eliminarImagen = async (id) => {
  ...
  const getMensajeTexto = (msg) => {
    return msg?.mensaje || msg?.contenido || msg?.texto || msg?.descripcion || '';
  };

  const getMensajeCorreo = (msg) => {
    return msg?.correo || msg?.email || msg?.telefono || 'Sin dato';
  };
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f0f4f8' }}>
      ...
      <div style={{
        ...
        {['publicaciones', 'galeria', 'mensajes'].map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            style={{
        ...
        {tab === 'mensajes' && (
          <div>
            ...
            {loadingMensajes ? (
              ...
              <p style={{ textAlign: 'center', color: '#94a3b8' }}>No hay mensajes aún.</p>
            ) : (
              <div style={{
                display: 'grid',
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          border: 'none',
                          borderBottom: '1px solid #e2e8f0',
                          background: mensajeActivo?.id === msg.id ? '#eff6ff' : 'white',
                          cursor: 'pointer'
                        }}
                      >
                        <div style={{
                          ...
                          <div style={{ fontSize: '0.72rem', color: '#94a3b8', whiteSpace: 'nowrap' }}>
                            {msg.created_at ? formatFecha(msg.created_at) : ''}
                          </div>
                        </div>

                        <div style={{ fontSize: '0.82rem', color: '#64748b', marginBottom: '0.35rem' }}>
                          {getMensajeCorreo(msg)}
                        </div>
                    ...
                    <>
                      <div style={{
                        ...
                        <h3 style={{ margin: 0, color: '#1e40af' }}>
                          {mensajeActivo.nombre || 'Sin nombre'}
                        </h3>

                        <p style={{ margin: '0.35rem 0', color: '#64748b' }}>
                          {getMensajeCorreo(mensajeActivo)}
                        </p>

                        {mensajeActivo.asunto && (
                          <p style={{ margin: '0.35rem 0', color: '#334155', fontWeight: '600' }}>
                            Asunto: {mensajeActivo.asunto}
                          </p>
                        )}

                        <small style={{ color: '#94a3b8' }}>
                          {mensajeActivo.created_at ? formatFecha(mensajeActivo.created_at) : ''}
                        </small>
                      </div>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'flex-start', flex: 1 }}>
                        <div style={{
                          ...
                          {getMensajeTexto(mensajeActivo) || 'Sin contenido'}
                        </div>
                      </div>
                    </>
                  ) : (
                    <p style={{ color: '#94a3b8' }}>Selecciona un mensaje.</p>
                  )}
                </div>
              </div>
            )}
